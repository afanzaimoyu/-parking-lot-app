	...
	<view class="search-car-all">
		...
		<view class="bg-icon" :class="set">
			...
			<view class="bg-text">
				...
				<view class="bg-text-bottom">输入完整车牌号，查询缴费情况</view>
			</view>
		</view>
		...
		<view class="search-car-input" @click="carInputClick">
			...
			<view class="search-car" @click.stop="addSearch">添加并查询</view>
		</view>
		...
		methods: {
			...
			addSearch() {
				...
				if (this.plateNum.length > 6) {
					...
					console.log("执行查询命令", this.plateNum);
					this.searchMoney(this.plateNum);
			...
			searchMoney(param) {
				...
				var url = "/pages/payment/payment?car=" + this.plateNum
				this.$tab.navigateTo(url);

				// await HowMoney(param).then(res => {
				// 	this.$modal.closeLoading();
				// 	this.$tab.navigateTo('/pages/payment/payment');
				// }).catch(err => {
